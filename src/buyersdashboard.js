    ...
    let products = [
      ...
      { id: 5, name: "Ripe Mangoes", price: 15.00, category: "fruits", farmer: "Tropical Gardens", image: "https://plus.unsplash.com/premium_photo-1674382739389-338645e7dd8c?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8cmlwZSUyMG1hbmdvZXN8ZW58MHx8MHx8fDA%3D", rating: 4.7, inStock: true },
      { id: 6, name: "Fresh Basil", price: 4.00, category: "herbs", farmer: "Herb Haven", image: "https://images.unsplash.com/photo-1662422325326-19089df23d98?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8ZnJlc2glMjBiYXNpbHxlbnwwfHwwfHx8MA%3D%3D", rating: 4.4, inStock: true }
    ];
...
function initiatePaystackPayment() {
  const total = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
  const email = document.getElementById('profile-email')?.value || 'customer@example.com'; // Fallback email
  const amountInKobo = total * 100; // Convert to kobo for Paystack (1 GHS = 100 kobo)

  const handler = PaystackPop.setup({
    key: 'pk_test_6952758f46a1beed53c3cea3edb76f22e6c1daec', // Replace with your Paystack public key
    email: email,
    amount: amountInKobo,
    currency: 'GHS',
    ref: 'AGRO' + Math.floor((Math.random() * 1000000) + 1), // Unique transaction reference
    callback: function(response) {
      ...
      const newOrder = {
        ...
        total: total,
        items: cart.length,
        paymentRef: response.reference // Store Paystack transaction reference
      };
